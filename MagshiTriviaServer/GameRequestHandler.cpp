...
GameRequestHandler::GameRequestHandler(RequestHandlerFactory* m_factory, LoggedUser m_user, Game m_game) {
	this->m_game = m_game;
...
RequestResult GameRequestHandler::submitAnswer(RequestInfo requestInfo) {
	...
	try {
		...
		std::string correctAnswer = this->m_user.getCurrGame()->getQuestionForUser(this->m_user).getCorrectAnswer();
		for (auto answer : this->m_user.getCurrGame()->getQuestionForUser(this->m_user).getPossibleAnswers()) {
			i++;
			if (answer == correctAnswer) {
				break;
			}
		}
		if (this->m_game.getPlayers()[this->m_user].wrongAnswerCount + this->m_game.getPlayers()[this->m_user].correctQuestionCount == 0) {
			this->m_game.getPlayers()[this->m_user].averageAnswerTime = requestInfo.receivalTime;
		}
		else if (this->m_game.getPlayers()[this->m_user].wrongAnswerCount + this->m_game.getPlayers()[this->m_user].correctQuestionCount == this->m_game.getQuestions().size()) {
			...
			this->m_game.getPlayers()[this->m_user].averageAnswerTime /= (long int)this->m_game.getQuestions().size();
		}
		...
		if (this->m_game.isFinished()) {
			for (auto player : this->m_game.getPlayers()) {
				this->m_factory->getStatisticManager().updateStatistics(player);
			}
		}
